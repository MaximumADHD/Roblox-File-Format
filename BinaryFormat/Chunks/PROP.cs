...
namespace RobloxFiles.BinaryFormat.Chunks
{
    public class PROP : IBinaryFileChunk
    {
        ...
        public void Load(BinaryRobloxFileReader reader)
        {
            ...
            var readInts = new Func<int[]>(() =>
                reader.ReadInterleaved(objCount, reader.RotateInt32)
            );
            var readFloats = new Func<float[]>(() =>
                reader.ReadInterleaved(objCount, reader.RotateFloat)
            );
            ...
            switch (Type)
            {
                                ...
                                default:
                                    {
                                        ...
                                        if (member != null)
                                        {
                                            ...
                                            memberType == typeof(ProtectedString)
                                            && File.HasSignatures
                                        )
                ...
                case PropertyType.OptionalCFrame:
                    {
                        ...
                        if (Type == PropertyType.OptionalCFrame)
                        {
                            ...
                            {
                                RobloxFile.LogError(
                                    $"Unexpected property type in OptionalCFrame (expected {cframeType}, got {readType})"
                                );
                        ...
                        if (Type == PropertyType.OptionalCFrame)
                        {
                            ...
                            {
                                RobloxFile.LogError(
                                    $"Unexpected property type in OptionalCFrame (expected {boolType}, got {readType})"
                                );
                                readProperties(i => null);
                                break;
                            }
                ...
                case PropertyType.Enum:
                    {
                        ...
                        readProperties(i =>
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    RobloxFile.LogError(
                                        $"Enum cast failed for {ClassName}.{Name} using value {value}!"
                                    );
                                    return value;
                                }
                ...
                case PropertyType.Ref:
                    {
                        ...
                        readProperties(i =>
                        {
                            ...
                            {
                                RobloxFile.LogError(
                                    $"Got out of bounds referent index in {ClassName}.{Name}!"
                                );
                                return null;
                            }
                ...
                case PropertyType.UniqueId:
                    {
                        var uniqueIds = reader.ReadInterleaved(
                            objCount,
                            (buffer, offset) =>
                            {
        ...
        internal static Dictionary<string, PROP> CollectProperties(
        ...
        public void Save(BinaryRobloxFileWriter writer)
        {
            ...
            switch (Type)
            {
                ...
                case PropertyType.Int:
                    {
                        var ints = props.Select(prop => prop.CastValue<int>()).ToList();
                ...
                case PropertyType.Float:
                    {
                        var floats = props.Select(prop => prop.CastValue<float>()).ToList();
